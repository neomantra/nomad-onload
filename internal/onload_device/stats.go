...
// TODO: this is not implemented, just the skeleton with nothing generated
...
func (d *OnloadDevicePlugin) doStats(ctx context.Context, stats chan<- *device.StatsResponse, interval time.Duration) {
	defer close(stats)
...
func (d *OnloadDevicePlugin) writeStatsToChannel(stats chan<- *device.StatsResponse, timestamp time.Time) {
...
func (d *OnloadDevicePlugin) collectStats() ([]*deviceStats, error) {
	d.deviceLock.RLock()
